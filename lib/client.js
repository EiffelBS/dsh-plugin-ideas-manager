window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var IdeasClient = class {
			...
			constructor(transport, workspacesSource) {
				this.transport = transport;
				this.workspacesSource = workspacesSource;
				if (this.workspacesSource !== void 0) {
					this.syncWorkspaces();
...
`;
		...
		*  restore), so the author can move an idea without leaving the editor. */
		function IdeaModal({ client, initial, initialWorkspace, onClose, onFollowUp }) {
			...
			const [workspace, setWorkspace] = (0, import_react.useState)(initial?.workspaceId ?? initialWorkspace ?? "");
		...
		*/
		const inject = [WORKSPACES_SERVICE];
		let claimed = false;
		const releaseClaim = () => {
			claimed = false;
		};
