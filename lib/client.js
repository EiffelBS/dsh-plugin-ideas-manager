window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		const IDEAS_OPEN_ORDERINGS = ["rank", "activity"];
		/**
		...
		*/
		const fr = {
			...
			"settings.openOrderingDesc": "Rang : le classement humain, exactement tel qu’il a été écrit — par défaut. Activité : dans la colonne Ouvertes de l’Aperçu, les idées dont une exécution est en cours remontent en tête, puis celles en échec, chaque bloc restant classé par rang. Ce tri est une vue : il n’enregistre jamais le classement, le glisser-déposer et les flèches écrivent toujours le rang humain.",
			"settings.openOrderingRank": "Rang",
			"settings.openOrderingActivity": "Activité (en cours d’abord)",
			"card.dragLocked": "Classement affiché par état : le glisser-déposer est désactivé sur cette colonne (un dépôt viserait un rang que la vue affiche ailleurs). Repassez l’ordre de la colonne Ouvertes sur « Rang » pour réordonner, ou utilisez les boutons de la carte pour changer de colonne.",
		...
		const en = {
			...
			"settings.openOrderingDesc": "Rank: the human ranking, exactly as it was written - the default. Activity: in the Overview Open column, ideas with a run in flight come first, then the failed ones, every block still rank-sorted. This order is a view: it never stores the ranking, drag & drop and the move arrows always write the human rank.",
			"settings.openOrderingRank": "Rank",
			"settings.openOrderingActivity": "Activity (running first)",
			"card.dragLocked": "This column is shown in state order, so drag and drop is off here (a drop would target a rank the view displays elsewhere). Switch the Open column order back to Rank to reorder, or use the card buttons to move an idea to another column.",
		...
		*/
		const zh = {
			...
			"settings.openOrderingDesc": "排名：完全按照人工排定的顺序 — 默认值。活动：在概览的“进行中”列中，有执行正在进行的想法排在最前，其次是失败的想法，每个区块内部仍按排名排序。该顺序只是一种显示方式：它从不写回排名，拖放与上下移动箭头写入的始终是人工排名。",
			"settings.openOrderingRank": "排名",
			"settings.openOrderingActivity": "活动（进行中优先）",
			"card.dragLocked": "该列按状态显示，因此这里禁用了拖放（一次放置会指向视图中显示在别处的排名）。将“进行中”列的顺序改回“排名”即可重新排序，或使用卡片上的按钮把想法移到其他列。",
...
`;
		...
		*/
		function orderOpenColumn(rows, grouped, workspaceTitle, ordering) {
			const within = (group) => ordering === "activity" ? orderByActivity(group) : orderIdeas(group);
...
${opinion}`;
		...
		* mirrors.
		*/
		function hasRunStateTags(idea, parentNumber) {
			if (idea.followUpOfId !== void 0 && parentNumber !== void 0) return true;
			...
			return idea.deliveredAt !== void 0;
		}
		/**
		...
		* unconditionally.
		*/
		function RunStateBadges({ idea, client, parentNumber, showDelivered = true }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
				...
				idea.runStatus === "running" && sessionId !== void 0 && sessionId !== "" && opener !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
		...
		*  label), with null for a workspace that has no registry title. */
		...
		function PrioritiesView({ client, openIdeas, allIdeas, workspaceTitle, onEdit, onToggleTag, activeTags, mdMode, grouped, parentNumber }) {
		...
		function DeliveredView({ client, archivedIdeas, workspaceTitle, onEdit, onToggleTag, activeTags, mdMode, parentNumber }) {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										...
										(workspaceId !== void 0 || idea.tags !== void 0 || idea.value !== void 0 || idea.effort !== void 0 || hasRunStateTags(idea, parentNumber)) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: classes.cardMeta,
											children: [
		...
		*/
		...
		function IdeasBoard({ client }) {
			...
			const byStatus = (status) => {
				...
				return status === "open" ? orderOpenColumn(rows, grouped, workspaceTitle, cfg.openOrdering) : grouped ? orderByWorkspaceGroups(rows, workspaceTitle) : orderIdeas(rows);
			};
			/**
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [status === "open" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
											...
											const workspaceId = idea.workspaceId;
											const dragLabel = status === "open" && !openColumnRanked ? t("card.dragLocked") : t("card.drag");
