	...
	factory: (require) => {
		...
		*/
		const fr = {
			...
			"settings.openOrderingDesc": "Comment les idées ouvertes sont rangées dans l’Aperçu. Une vue d’affichage uniquement : aucun rang n’est enregistré, le classement que vous fixez au glisser-déposer est conservé.",
			...
			"settings.runningFirstDesc": "Fait remonter les idées dont une exécution est en cours au-dessus de l’ordre choisi ci-dessus, sans le modifier. Tant que cette option est cochée, l’ordre affiché diffère du rang enregistré : le glisser-déposer est donc désactivé sur cette colonne.",
			"card.dragLocked": "Cette colonne n’affiche pas l’ordre enregistré (tri par date ou idées en cours remontées) : le glisser-déposer y est désactivé, car un dépôt viserait un rang affiché ailleurs. Repassez l’ordre sur « Rang » et désactivez l’option « en cours d’exécution en haut » pour réordonner, ou utilisez les boutons de la carte pour changer de colonne.",
		...
		const en = {
			...
			"settings.openOrderingDesc": "How the open ideas are laid out in the Overview. A display view only: no rank is stored, so the ranking you set by drag & drop is preserved.",
			...
			"settings.runningFirstDesc": "Floats the ideas whose run is in flight above the order picked above, without changing that order. While this is on, the displayed order differs from the stored rank, so drag & drop is off on that column.",
			"card.dragLocked": "This column does not display the stored order (a date order, or running ideas floated to the top), so drag & drop is off here: a drop would target a rank the view displays elsewhere. Set the order back to Rank and turn off \"Show running ideas at the top\" to reorder, or use the card buttons to move an idea to another column.",
		...
		*/
		const zh = {
			...
			"settings.openOrderingDesc": "概览中打开的想法如何排列。这只是一种显示方式：不会保存任何排名，因此你通过拖放设定的排名会被保留。",
			...
			"settings.runningFirstDesc": "把执行正在进行的想法放到上面所选顺序之前，而不改变该顺序。启用时，显示顺序与已保存的排名不同，因此该列的拖放会被禁用。",
			"card.dragLocked": "该列显示的不是已保存的顺序（按日期排序，或置顶正在执行的想法），因此这里禁用了拖放：一次放置会指向视图中显示在别处的排名。将顺序改回“排名”并关闭“将正在执行的想法置顶”即可重新排序，或使用卡片上的按钮把想法移到其他列。",
		...
		*/
		...
		function IdeasBoard({ client }) {
			...
			const byStatus = (status) => {
				...
				return status === "open" ? orderOpenColumn(rows, grouped, workspaceTitle, cfg.openOrdering, cfg.runningFirst) : grouped ? orderByWorkspaceGroups(rows, workspaceTitle) : orderIdeas(rows);
			};
			/**
