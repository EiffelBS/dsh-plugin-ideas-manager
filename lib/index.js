...
function validReadQuery(input) {
	...
	return (input.workspaceId === void 0 || input.workspaceId !== "" && input.workspaceId.length <= 256) && (input.status?.length ?? 0) <= 100 && (input.ids?.length ?? 0) <= 100 && (input.numbers?.length ?? 0) <= 100 && (input.fields?.length ?? 0) <= IDEAS_READ_SELECTABLE_FIELDS.length && statusValid && idsValid && numbersValid && fieldsValid && bounded(input.bodyLimit, 0, 4096) && bounded(input.limit, 1, 200) && bounded(input.offset, 0, 1e6);
}
/**
...
var IdeasHostService = class {
	...
	*/
	async pollRunTransitions() {
		...
		for (const idea of this.ledger.snapshot().ideas) {
			...
			if (observed !== void 0 && observed !== idea.taskBoardStatus) try {
			...
			if (observed !== void 0 && observed !== idea.runStatus) try {
	...
	*/
	async launchIdea(ideaId, model, requestId) {
		if (!this.active) throw new Error("ideas plugin is disabled");
		if (this.mirror === void 0 || !this.autoMirror) throw new TaskBoardMirrorDisabledError();
...
var TaskBoardMirror = class {
	...
	*/
	async launchTask(idea, model) {
		...
		if (chosen !== void 0 && chosen !== "") await this.post({
			kind: "update",
			taskId,
			patch: { model: chosen }
		});
	...
	return [
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				if (parsed.model !== void 0 && parsed.model.length > LAUNCH_MODEL_MAX_LENGTH) return deny(400, "model-too-long");
				try {
