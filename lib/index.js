	...
	apply(action) {
		...
		switch (action.kind) {
			...
			case "decline": {
				const idea = this.document.ideas.find((item) => item.id === action.ideaId);
				if (idea === void 0) throw new Error("idea not found");
				if (idea.status !== "declined") this.document.ideas = this.document.ideas.map((item) => item.id === action.ideaId ? {
...
const IDEAS_GUIDANCE = "dsh-plugin-ideas-manager is installed (generic idea manager, \"Ideas\" board in the sidebar under New Session): Host-authoritative /api/ideas ledger per workspace; capture ideas and triage them on a 3-column kanban (open / archived / declined) with tags, value/effort scores and manual rank; one-directional markdown export (ledger -> IDEAS.md / IDEAS-ARCHIVE.md); optional TaskBoard mirror when the task-board plugin is present (create -> backlog, move, decline -> archive; closing the loop to done is manual). The board is autonomous without the task-board plugin. When the user mentions ideas / backlog / idees / notes, collaborate through this board.";
/**
