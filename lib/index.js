...
function parseSettingsBody(value) {
	...
	if (patch === void 0 || !exactKeys(patch, ["tagRows"])) return void 0;
	if (patch.tagRows !== void 0 && (typeof patch.tagRows !== "number" || !Number.isFinite(patch.tagRows))) return void 0;
	return {
		patch: patch.tagRows === void 0 ? {} : { tagRows: clampTagRows(patch.tagRows) },
...
var IdeasHostService = class {
	...
	*/
	enqueueMirror(ideaId, kind, idea) {
		const run = (this.mirrorChains.get(ideaId) ?? Promise.resolve()).catch(() => void 0).then(async () => await this.runMirror(kind, idea));
	...
	*  brick the namespace — the clamp keeps a hand-edited file working). */
	...
	ctx.inject(["settings"], (sctx) => {
		...
		const viewOf = () => {
			...
			return {
				available: true,
				value: { tagRows: clampTagRows((descriptor.value ?? {}).tagRows) },
		...
		configPort = {
			...
			write: async (patch, expectedRevision) => {
				const section = patch.tagRows === void 0 ? {} : { tagRows: clampTagRows(patch.tagRows) };
